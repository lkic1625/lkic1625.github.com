...
벌크 액션 서버를 짜고 이후 관리하면서 생긴 후일담에 대한 개인적인 회고록입니다. 최근에 팀 내에서 [급증하는 트래픽 안정적으로 처리하기](https://channel.io/ko/team/blog/articles/235661b0)라는 내용으로 테크 블로그 글을 작성했습니다.

---
그 이후로, 논리적 파티셔닝과 동적 워커수 스케일링도 개선했습니다. 혹시나 관심이 있으시다면 아래 글을 참고해주세요. 
- [급증하는 트래픽 안정적으로 처리하기: 개선편(2) 논리적 파티셔닝](https://channel.io/ko/team/blog/articles/tech-logical-partitioning-0db50646)
- [급증하는 트래픽 안정적으로 처리하기: 개선편(1) 동적 스케일](https://channel.io/ko/team/blog/articles/tech-dynamic-scaling-6748dc7e)
---
...
## 시스템의 복잡도 어디까지 희생할 것인가

우리는 결국 종-횡적 확장을 위해, 논리적 파티셔닝과 동적 워커수 스케일링 등을 적용했다. 동적 워커수 스케일링만으로 끝날 수도 있지만, 이렇게 했을 때는 결국 pod 수 역시 고정해야지 일관된 스케일링 기준을 확보할 수 있다. 그렇기에 논리적 파티셔닝 또한 적용하게 되었는데, 이게 진짜 시스템 복잡도를 많이 희생하는지 저울질 하는 게 재밌는 고민이었다.

기존에도 파티셔닝된 인프라를 리밸런싱 한다던가, 밸런싱 기준이 일관적이지 않아 서버 인프라 마비 등을 겪어본 입장에선 파티셔닝을 도입한다는 게 얼마나 운영 복잡도를 추가하는지 알고 있기 때문이다. 그렇기에 파티셔닝 수를 고정하고, 카프카의 KRaft 처럼 기존 구현을 최대한 활용하는 형식으로 구현과 운영 복잡도를 낮추려고 했다. 결국 실제 목표는 "내가 없어도 알아서 잘 굴러가는 시스템"을 목표로 했기 때문이다. 

물론, 이 trade-off 하에서 의도적으로 과제도 몇 가지 남기고 왔다. 과하지 않게 적절한 문제를 정의하고 풀지 않을 문제를 정의하는 능력 또한 엔지니어에게 정말 중요한 영역이라고 생각한다.

## 결국 "사람"과 함께 한다는 게 중요하다

우리가 엄청난 시스템을 만든다고 해도 그걸 설득하고, 이를 풀어과는 과정이 정말 어렵다고 생각한다. 벌크액션을 개발할 때도 나는 내가 팀에서 쌓아온 신뢰를 기반으로 이해당사자를 설득하고, 뒷받침되는 논리로 엔지니어링을 펼쳤다.(나는 이걸 신뢰자산이라 표현한다.)

완벽하고 논리적인 무기로 누군가를 함구하게 하는 설득이 아니라, 결국 이를 도입해야하는 사용자들에게 하나하나 찾아가 설득하고 이해당사자들을 내편으로 끌어오는 것이 벌크액션을 개발하는 과정에서 엔지니어링만큼 중요한 과제였다고 생각한다. 

## 결론

두서 없이 생각나는 걸 몇 가지 적어보았다. 아마도 이러한 생각은 또 언젠가 달라질 수도 있겠지만 지금 이 순간, 내가 생가하는 일에서, 엔지니어로서 중요한 점을 나열해보았다.
