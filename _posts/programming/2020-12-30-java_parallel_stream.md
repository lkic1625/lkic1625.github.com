...
병렬 스트림은 내부에서 `ForkJoinPool` 을 이용해 구현되었다.

`ForkJoinPool`이란 자바 7 버전 이후 추가된 기능이며, 한 작업(task)을 여러 작은 작업(sub task)으로 나누어 각각 다른 스레드에서 연산할 수 있도록 한다.
![이미지](/assets/images/ForkJoinPoolStructure.png)
작업은 나뉘어져 일종의 이진 트리를 생성하며, 종단 노드는 같은 양의 연산을 하게되는데, `Divide And Conquer` 알고리즘과 같은 방식으로 보이기도 하며, 병렬 스트림은 이를 이용해 구현되었다.

`ForkJoinPool`은 내부에 `Work Stealing` 알고리즘을 사용하는 등 흥미로운 구현들이 많지만, 이번 포스트 주제를 벗어나는 내용인 것 같아 자세히는 설명하지 않겠다.
...
`ArrayList`의 경우에는 병렬 스트림을 사용하기 가장 적합한 예제로 볼 수 있다. 내부에서 이미 해당 리스트의 크기를 알고 있기 때문에 쉽게 중간점을 찾을 수 있고, (균형잡힌 이진 트리로) 쪼개기 적합하다.

반면에 `LinkedList`는 중간점을 찾기 위해 전체 리스트의 절반을 순회해야 하는 문제점을 가지고 있다. 이렇게 된다면 나누는데 항상 $$O(n)$$ 의 시간이 걸리게 된다.
<br>
`LinkedList`는 이러한 분할 연산의 비효율적인 구조를 피하기 위해 불균형 이진 트리를(unbalanced binary tree)를 생성하게 되는데, 불균형 이진 트리를 생성하게 되므로 분할과 집계 과정에 $$O(n)$$이 소요된다. (`ArrayList`는 $$O(logN)$$이다.)
...
연산 트리를 마지막으로 합칠 때, `sorted`, `collect` 등의 메서드는 `sequential`한 작업이기 때문에 이 또한 병렬 스트림의 성능 저하 원인이 될 수 있다.
...
![이미지2](/assets/images/memory-hierarchy.png)
...
자바 구현의 경우 대부분 객체의 필드를 메인 메모리에 연속적인 구조로 배치하고 있다. 그에 따라 `primitive` 한 타입의 필드를 가지고 있다면 메모리 지역성이 매우 높아 캐싱에 큰 이점이 있을 것이다.
