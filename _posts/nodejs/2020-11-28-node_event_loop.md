...
만약 더 이상 실행할 콜백이 없다면,  `check_queue`, `pending_queue`, `closing_callbacks_queue에` 해야할 작업이 있는지를 검사하고, 만약 해야할 작업이 있다면 바로 `Poll phase가` 종료되고 다음 페이즈로 넘어가게 된다. 하지만 특별히 해야할 작업이 더 이상 없는 경우 `Poll phase`는 다음 페이즈로 넘어가지 않고 계속 대기하게 된다.
ㅔㅔ
